...
class AccountDetailPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final current = accounts.where((a) => a.id == account.id).firstOrNull ??
        account;
    final snapshots = ref.watch(snapshotsProvider).value ?? const [];
    final accountSnaps =
        snapshots.where((s) => s.accountId == current.id).toList();
      ...
      body: ListView(
        ...
        children: [
              ...
              child: Row(
                children: [
                  CircleAvatar(
                    radius: 24,
                    backgroundColor:
                        hexToColor(current.color).withValues(alpha: 0.15),
          ...
          FilledButton.tonalIcon(
            ...
            label: const Text('调账（设为目标余额）'),
          ),
          const SizedBox(height: 16),
          Text('历史快照（${accountSnaps.length}）',
          ...
          if (accountSnaps.isEmpty)
            ...
            ...accountSnaps.reversed.map((s) => ListTile(
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context)
          ..hideCurrentSnackBar()
          ..showSnackBar(SnackBar(content: Text('$e'.replaceFirst('ValidationException: ', ''))));
      }
    }
  }
