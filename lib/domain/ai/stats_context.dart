...
class AiStatsContext {
  ...
  Future<String> build({int trendMonths = 6}) async {
    ...
    final categories = _ref.read(categoriesProvider).value ?? const <Category>[];
    final catName = {
    ...
    try {
      ...
      final cur = await billRepo.summaryInRangeOnce(
        thisMonth.$1, thisMonth.$2,
      );
      final prev = await billRepo.summaryInRangeOnce(
        lastMonth.$1, lastMonth.$2,
      );
      buf.writeln('\n### 月度收支');
      buf.writeln('- 本月：支出 ${_yuan(cur.expense)}，收入 ${_yuan(cur.income)}，'
          '结余 ${_yuan(cur.income - cur.expense)}');
      buf.writeln('- 上月：支出 ${_yuan(prev.expense)}，收入 ${_yuan(prev.income)}，'
    ...
    try {
      ...
      final expenseTop = await billRepo.sumByCategoryInRange(
        range.$1, range.$2, BillType.expense,
      );
      final incomeTop = await billRepo.sumByCategoryInRange(
        range.$1, range.$2, BillType.income,
      );
      if (expenseTop.isNotEmpty) {
        buf.writeln('\n### 本月支出分类 Top10');
        for (final e in (expenseTop.toList()
              ..sort((a, b) => b.amount.compareTo(a.amount)))
            .take(10)) {
          buf.writeln('- ${catName[e.categoryId] ?? e.categoryId}：${_yuan(e.amount)}');
        }
      }
      if (incomeTop.isNotEmpty) {
        buf.writeln('\n### 本月收入分类 Top10');
        for (final e in (incomeTop.toList()
              ..sort((a, b) => b.amount.compareTo(a.amount)))
            .take(10)) {
          buf.writeln('- ${catName[e.categoryId] ?? e.categoryId}：${_yuan(e.amount)}');
        }
    ...
    try {
      ...
      for (var i = trendMonths - 1; i >= 0; i--) {
        ...
        buf.writeln('- ${m.year}/${m.month}：支出 ${_yuan(s.$1)}，收入 ${_yuan(s.$2)}');
      }
    } catch (_) {}

    // 5) 资产趋势（近 90 天，周粒度）
    try {
      final snaps = _ref.read(snapshotsProvider).value ?? const <BalanceSnapshot>[];
    ...
    try {
      ...
      if (items.isNotEmpty) {
        ...
        for (final it in items) {
          ...
          buf.writeln('- ${it.name}：已用 ${_yuan(it.spent)} / ${_yuan(it.amount)}（$pct%）');
        }
      }
    } catch (_) {}
